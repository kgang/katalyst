...
## Second amendment (2026-09-21) — a twentieth code: one ordered pair of claims takes one arrow

**The map's rules gain `duplicate_link`.** A second arrow between the **same ordered pair** of claims — the same cause, the same effect, the same way round — is a fault in the map, found by `validate` like any other. Rule 9 in `spec/graph/validity.md`, test `test_validate_rejects_a_second_arrow_between_the_same_two_claims`. **Both directions stay legal:** an arrow from A to B and an arrow from B to A are two different pairs, and the loop rule (rule 10) already says what may and may not come of that.

**Why it is a fault and not a curiosity.** Working the likelihoods through **adds every incoming arrow's push together**. So two arrows the same way round push twice as hard as either of them claims, and nothing in an arrow's fields can say *these are two separate channels* — an arrow carries one mechanism, one strength, one delay and one shape. The validator's own sentence says it in the reader's words: *"… and … are already joined by an arrow in that direction. Two arrows the same way round add their pushes together, so a second one says the cause is stronger than either arrow claims."* When there really are two mechanisms, the map has a way to say so already: put the middle claim in between, and the two channels become two paths a reader can argue with one at a time.

**The first arrow between a pair is never the fault; the ones after it are.** That is what lets a map be built one arrow at a time — the arrow that arrives and finds the pair already joined is the one refused, and the one the interface can point at.

**Where it came from.** An independent review of the generation pipeline, 2026-09-21: claims on the frontier are expanded concurrently, so two rounds in flight can propose the same arrow, and each is legal on the map it was asked about. Nothing before this rule would have caught the pair once both landed. This is the ordinary shape of every rule in this record — the model proposes, our code disposes, and a fault the model can produce is refused by tested code rather than requested in a prompt.

**What has not changed.** Nothing in the decision. This is the fifteenth fault `validate` can find, beside the five refusals our own code makes; `validate` still returns **every** fault at once, still names claims in their own words, and still rejects rather than repairs — the pipeline never drops the second arrow quietly, it shows the refusal and asks again.
