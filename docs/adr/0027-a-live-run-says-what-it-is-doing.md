...
* **Dated correction, 2026-09-22 — the invariant this record names is `INV-generation.33`, not `.22`.** The `spec-impact` line above and the *Confirmation* section were written citing `INV-generation.22`, which was already `replay.md`'s *a replay emits the recording, line for line*. The chapter's numbers come from one pool shared by the five chapters of that part, so two invariants cannot carry one number. The invariant itself is unchanged and so is every word of this record's reasoning; only the number it is filed under moved, to `.33`, the next free one. The original sentences are left as they were written, which is what a journal is for.
