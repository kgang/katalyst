...
* **Amended 2026-09-21, proposed with record 0024 and in force only when Kent accepts it.** The decision above says already-placed nodes are pinned, *"so a newly streamed tile never reshuffles an existing layer"*. That held on every map this record was written against, and it is what drew arrows pointing backwards on a generated map: the pin held a tile's column as well as its row, so a claim whose causes arrived after it stayed left of them (on two live runs of 2026-09-21, 3 of 9 arrows and 2 of 5, measured through the app's own layout code). Record 0024 keeps the pin on the **row** and frees the **column**: a tile moves only when an arrow would otherwise point backwards, and once when the run stops. The layout engine, its options and the background thread are unchanged; nothing else in this record moves.
