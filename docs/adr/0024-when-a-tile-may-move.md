...
1. **A tile keeps its row.** Its vertical place, and its order within its column, are pinned from the moment it is first drawn and are never taken from it while the map grows. This is INV-workbench.23 and it is unchanged — it is now the half of the pin that does all the work. **With one exception, and it is the arrows' doing (added 2026-09-22, from building it).** When a later arrow sends a tile into a column where the row it holds is already taken, it takes the **nearest free row** in that column — its own row whenever that row is free, otherwise the closest one that is, up or down, a tie going down, which is reading order. Measured by the implementing agent on 2026-09-22, folding both streams event by event through the app's own layering, layout engine and reading-back: this happens **once in the whole of the live Verify run, by 48 pixels, and not at all on the eighteen-claim committed recording.** Holding the row regardless was built first and measured: on the recording it drew one claim **140 pixels on top of another** for four consecutive events. Taking the *first* free row below was built and measured too: it moved the Brent tile **392 pixels down past a free row 48 pixels above it** — correct, and further than a tile a reader is watching should travel. A reserved rectangle is a box for this purpose, never room to be drawn over.
...
* `test_a_tile_sent_into_a_taken_row_takes_the_nearest_free_row` and `test_a_tie_between_two_free_rows_goes_down` — the one exception to the row, on built maps.
