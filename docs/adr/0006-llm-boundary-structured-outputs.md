...
* **Grounding.** The `web_search_20260209` server-side tool is declared on calls that need evidence; `code_execution` is *not* also declared, since dynamic filtering already runs it. Citations from the results fill the link's `sources`; a link that turns up nothing is marked `asserted`, never `documented`. *(Amended 2026-09-17: `documented` needs at least one cited source that the search tool itself returned in that same call, and an arrow that turns up nothing comes out `argued` rather than `asserted`. See the amendment at the foot of this record; the original sentence stands as written. A second amendment, dated 2026-09-20, adds the research budget for a claim's base rate.)*
...
## Amendment (2026-09-17) — what counts as a source

**The grounding bullet's last clause becomes precise in two ways. The rule it states — the model never talks its way to `documented` — does not change.**

**(a) `documented` only from a source the search tool itself returned in that call.** Kent settled this on 2026-09-17. A citation counts only when its address is one the server-side search tool returned **during the same call**. An address the model typed that the tool never returned is **not a source**: it is dropped, and the transcript carries a note saying so. The reason is plain — a model asked for citations will produce citations, and an address that opens nothing, or opens something else, is worse than no address at all, because it invites a reader to check us and then embarrasses them. `spec/generation/grounding.md` anti-patterns 1 and 4 state this rule and the test `test_provenance_is_written_from_what_was_found` compares what we wrote against the tool's own result set.

**(b) An arrow that turns up nothing comes out `argued`, and no arrow this pipeline accepts is ever `asserted`.** `asserted` means no mechanism and nothing kept — but **every arrow must carry a rationale**, which is one of the map's own rules (violation code `missing_rationale`), and **we write provenance before the map is checked**. So an arrow with an empty rationale would be stamped `asserted` and refused a moment later; it never reaches a map. Every arrow this pipeline accepts is therefore `documented` or `argued`. `asserted` survives only where it always has: on maps a person wrote by hand — the shipped example's `H → N1`, marked that way by a human who judged its own sentence to be a story rather than a mechanism — and later on a probe's output. **Our code makes no such judgement about prose, and must not.** Written out in full, with worked examples, in `spec/generation/grounding.md` B3 and B4.

**What has not changed.** The model never declares its own provenance (ADR-0003, rule 5); we write the word from what actually happened; the word is written **once**, from the facts, and never marked up or down afterwards. The bullet's *"never `documented`"* half stands exactly as written: nothing that turned up nothing is ever marked `documented`, and a `documented` arrow that cites nothing is refused by the validator whoever wrote it — `test_expand_rejects_a_documented_arrow_that_cites_nothing` pins both halves of that seam. Only the name of what an empty-handed arrow is marked **instead** becomes precise, and only because two later rules — S1's tool-returned test and the rationale requirement — settle it.

Amended in place rather than superseded, because nothing in the decision changed: one word in a consequence was replaced by a more exact one.

## Second amendment (2026-09-20) — the model is a setting, and base rates are researched

Both of these were decided by Kent on 2026-09-20, **after the first live generation had been measured** — that run was made on 2026-09-17 and is quoted below with its own date. The decision this record carries — small structured calls, one proposal per call, composed by our own code — is untouched by either.

### (a) The model is one named setting, and the prototype runs on `claude-sonnet-5`

The prototype's default model becomes **`claude-sonnet-5`**, to lower what a run costs; **`claude-opus-5` is one setting away** for a final recording if quality asks for it. Four consequences, all small:

* The model name is **one setting**, read where every setting is read, and **nothing else in the pipeline knows which model it is**.
* `engine/pricing.py` holds a price **per model**, with the day the prices were read written beside them. It holds nothing else.
* **The prompt hash does not depend on the model.** It covers the prompt, so changing models does not stale every cassette and every recording, and two runs of the same prompt on different models compare cleanly.
* **The receipt and the recording both name the model**, so any number on screen can say which model produced it (NFR-6 already asks the receipt to carry it).

**The measured fact this rests on, as a dated measurement.** The first live generation of the Hormuz example, 2026-09-17, on `claude-opus-5` at default thinking effort: **10 calls, 9 searches, $1.32, 10 minutes 54 seconds**, and **61 per cent of the tokens written were thinking tokens**; it produced 10 claims and 9 arrows, refused nothing, and stopped at the width cap. **No figure for `claude-sonnet-5` exists yet, and none is stated here** — the first one is measured the first time it runs and written into `STATUS.md` beside this one.

The record's original instruction stands and matters more than ever now that a name is a setting: **verify every one of them against the `claude-api` skill before typing it** — the structured-output call, adaptive thinking, the `web_search_20260209` tool name, and the minimum number of tokens a prefix must reach before it can be cached. Never from memory. The `model="claude-opus-5"` in the *Client library and model* bullet now reads as the other setting rather than the default.

### (b) A base rate is researched, and is shown only when it cites what the research returned

**The evidence.** In that same measured run, **8 of the 10 claims came back carrying a count — "12 of 15" and the like — with no source at all.** A count nobody can go and recount is exactly the state this product refuses to show, and it is worse than an empty slot, because it looks like homework.

So the model **researches** the reference class rather than recalling it: **up to twenty-five web searches across up to five rounds, per claim call**, where a round is the model searching, reading what came back, and deciding whether to search again. The prompt tells it to stop as soon as it has a countable reference class with a source.

**A base rate is shown only when it cites a page the research actually returned** — the same test an arrow's sources must pass under the first amendment above. A count citing nothing, or citing an address the search never returned, is **dropped with a note in the transcript**, and the slot says honestly that there is no base rate. Nothing is invented to fill it.

**What this supersedes.** The budget of about one search per proposal — written down in `spec/generation/grounding.md` B6 rather than in this record — no longer holds for a claim call, and the cap on searches per generation rises to fit. Two things do **not** move: the **$15 hard stop per run** stays as the safety net, ending a generation with a plain sentence naming what was spent and what was got; and because a web search is billed apart from tokens, a run's dollars still cannot be re-derived from its token counts, which is why the receipt carries the search count beside them.

Amended in place rather than superseded, because neither change touches the decision: one call still takes one thing and returns one thing, our code still disposes, and the only things that moved are which model is the default and how hard a call works before it answers.
