...
**The move.** Click any box and change it: "…but Iran is struck the next day." The original map is kept. A new branch forks off. Only the boxes downstream of your change update. A faded overlay shows exactly what moved. Do this as often as you like and compare branches.
...
| D2 | Truth source | The model reasons out loud: every arrow states its mechanism, a base rate (how often this kind of thing has happened before), and **one likelihood**. Where a real market prices the same event, that price sits beside the model's number. **Stretch:** pick any box and spend extra compute on it — thousands of random simulations, a small probability model, or role-played experts hunting for gaps — so the user decides where precision matters. *(Amended 2026-09-22; decision record 0028. This read "a likelihood* range*, not a point" until Kent cut the range from the product: one likelihood per claim, computed once, and no range anywhere.)* | 0003, 0005, 0028 |
...
| **Link** (edge) | A causal claim from one proposition to another: `mode` (`trigger` — domino, fires once, decays; `sustain` — desk-holds-apple, the effect holds only while the cause holds), `strength` (how much it shifts the odds of the next proposition; on a log-odds scale so several links add up), `lag`, `shape` (`impulse` \| `step` \| `ramp`), `half_life`, `rationale`, `sources`, `provenance`. **Automatic retraction is gone** *(built 2026-09-22; decision record 0017)*: nothing on this product undermines a claim by itself, and the list of retractions a world reports is always empty — the words *"effect retracts if cause is removed"* stood here until 2026-09-21. **Still planned, and riding stack 05's shape freeze** (same record): the rule that a `sustain` arrow may leave only a state, which the arrow's own field description states and which nothing yet refuses |
...
| **Belief** | A probability and an owner: `model` \| `user` \| `market`. The three are never merged. **No belief carries a range** *(2026-09-22; decision record 0028: one likelihood per claim, computed once)* — the fields `lo` and `hi` are still on the wire, each of them now equals `p`, nothing reads them or prints them, and one later pull request takes the three fields down to one |
...
- **FR-2 (P0)** Ask for the user's own likelihood estimate at input as a **single slider** with an explicit "I don't know" state — while that state holds, the request carries no likelihood at all. Stored as a `user` belief on the hypothesis; never overwritten by the model. *(Amended 2026-09-22; decision record 0028. It was a range slider with three handles — a likelihood and two ends — and the reader is now asked for the one number, because no number on this product carries a range.)*
...
- **FR-12 (P0)** Provenance is visibly encoded on every link and belief chip (INV-2, UX-6). It is a mark that says where a number came from, and it sets nothing else. *(Amended 2026-09-22; decision record 0028. A second sentence stood here: provenance also set **how wide an arrow's push was drawn** when the engine worked the map through — narrow where a page the search returned backed the arrow, widest where nothing did (added 2026-09-21; decision record 0014). That width existed only to make the two thousand versions of the map differ from one another, and there are no versions: an arrow's push is its stated strength, used once.)*
...
- **FR-16 (P0)** Diff: ghost overlay of A under A′ in a shared union layout; delta rail of terminal changes ranked by **the size of the move × the weakest backing on the best-backed route** from a differing edit's subject to that terminal — over every such route, the one whose weakest arrow is strongest (amended 2026-09-17; "shortest path" and "which edit" both leave the rule). A claim counts as **shifted** when it moved by .005 or more, **and that is the whole test**; a claim that is not `shifted` says the one reason it failed, that it barely moved. The floor stays inside the engine and is on no wire, so nothing outside it can re-run the test and disagree. One-line natural-language diff. *(Amended 2026-09-22; decision record 0028, one likelihood per claim and no range anywhere. Three things left this requirement with the two thousand versions of the map they were made of: the rail's two side columns, **how firm** — a claim's range width — and **agreement**, the share of versions that moved the same way; and the second half of the shifted test, which asked for that share to reach 90%. The ranking on two factors is untouched, because neither factor was ever a version.)*
...
- **FR-21 (P2)** "Where to spend modeling budget": rank propositions by **each stated range's share of the terminal's band** — how much of the width on the number you care about comes from not being sure of *this* claim — and say so in the UI. *Not built in version one — decision record 0028: the quantity it ranks is a share of a range, and there is no range.* *(Amended 2026-09-22. This used to point at two lines of the generated file [`docs/worked-numbers.txt`](docs/worked-numbers.txt), `B · base · band from B` and `B · base · reading`, for the share and the band it was a share of. Neither line exists any more: the engine computes one likelihood per claim, computed once, so there is no band and nothing to take a share of. Corrected 2026-09-17 before that: the requirement used to say "base rate", which the Brent claim does not have.)*
...
### 6.7 Thesis
- **FR-25 (P0)** Thesis card, in six sections: **what carries it** · **what is priced in** (the model's number, the venue's bid and offer with their source and day, the fee, the gain from buying and from selling — or the named refusal and the break-even) · **what takes you out** · **what to watch**, with *unhedgeable* beside it · **your exit** (stop, target, horizon, risk budget — the reader's — and beside them a **greyed quartered-Kelly ceiling** worked out at the model's own likelihood, labelled *never size to this*, which reads zero where neither buying nor selling is worth it at the venue's price once the fee is paid, and is absent with a reason wherever no edge can be built) · **tails and reader-placed shocks**; plus the distribution (P1), the caveats (weakest arrows, unhedgeable claims, crowding), and one line of execution: *a stop order's trigger is not its fill price* (decision records 0018 and 0019). *(The ceiling's wording amended 2026-09-22; decision record 0028. It read "at the unfavourable end of the model's stated range … reads zero where that range does not agree which side of the price to be". There is no range, so the unfavourable end of it is the likelihood itself: the rule is unchanged and reads one number instead of two, and the disagreeing case can no longer arise.)*
- **FR-26 (P1)** Read-only prices, **recorded first**: a committed dated quote file the demo and the build run on, an opt-in live Polymarket read that falls back to it and never runs in continuous integration, and a reader-entered price always available. A venue's quote is shown beside the model's and the reader's numbers as the `market` belief — a **point**, never a range, because the difference between the best bid and the best offer is what dealing costs and the venue publishes no interval *(2026-09-22; decision record 0028: no number on this product carries a range now, so this is no longer the one exception. The sentence is kept because it is still the true answer to why a price is one number)* — and never as a bare number: venue, value, source and date are one unit. **A FRED figure is an observation, not a belief**: it anchors the spot level of a price ending, renders the required attribution line ("This product uses the FRED® API but is not endorsed or certified by the Federal Reserve Bank of St. Louis"), and never fills the `market` slot. **Kalshi is cut from version one** (decision record 0020); where one venue quotes a claim the card names it and says no other was asked, and where two ever do, both are shown and never averaged.
...
- **UX-8 Streaming is the loading state.** No spinner. Skeleton tiles appear at their layer, claims stream in, wires draw, chips resolve last. Reasoning arrives as it is produced. **And the screen says, in words, what it is waiting for and how long it has been waiting** (amended 2026-09-21; decision record 0023). **Silence is not a loading state**: on a live run the first proposal takes about twenty-three seconds to come back and later ones fifty to a hundred and ten seconds apart, so one strip at the foot of the map carries the run's own sentence and, whenever the screen is waiting on the server — a live run, or a stored map whose world is being worked out — the count of seconds since the last event. It never counts on a replay, whose pace we set ourselves. That count is a measurement and never an estimate of what is left; nothing spins, pulses or sweeps beside it.
...
- **NFR-7 Performance.** 60 tiles render and re-layout in <100ms on a laptop; **streaming first paint within 1 s of the request being accepted — a reserved rectangle at its column; this pipeline returns whole proposals, not tokens** (amended 2026-09-20: the old wording read "within 1s of the first token", and there are no tokens to paint — a call comes back with a finished proposal, seconds later, so what must appear inside the second is the space the first claim will land in). **Recalculating a branch is targeted proportional to the size of the map, not to a fixed number** — the reference point is **600 ms** for one world on a **twenty-claim** map, which is **30 ms a claim** by plain division, and what it protects is that an edit feels responsive. It is a target and not a gate: the engine's measured figures are recorded beside it in `ARCHITECTURE.md` §10, and no pull request is refused for missing it *(added 2026-09-21; Kent, R24, which supersedes the fixed 600 ms of R19; decision record 0016)*. *(Amended 2026-09-22; decision record 0028: the reference point used to say "for one world **with its range** … **at 2 000 versions**", and a world has neither any more. The figure is unchanged — it is a number somebody chose, proportional to the size of the map — and the dated measurements it is read against are in [`docs/measurements.md`](docs/measurements.md).)*
...
| INV-7 | **Honest numbers.** Every belief's low end, likelihood and high end are one and the same number after any sequence of interventions — `lo == p == hi`, and each of the three lies between 0 and 1 — and it is rendered at two significant figures and nothing else *(rewritten 2026-09-22; decision record 0028. The invariant read "0 ≤ low ≤ p ≤ high ≤ 1 … rendered with its range", which one likelihood satisfies without saying anything; the checkable statement is now that the three fields agree, and it stands until the two spare ones leave the wire)* | `spec/graph/`, `spec/workbench/` |
...
9. Do not ship a force-directed hairball, a spinner-then-dump, a modal, or the component library's default look — each is a D5 veto condition. **What a spinner-then-dump means is motion standing in for a sentence; a measurement that changes because time passed is not one** (amended 2026-09-21; decision record 0023). A count of seconds since the last event, beside a sentence that is complete without it, is allowed and is the only ticking thing in the product; a spinner beside that sentence is still the veto.
