...
- **FR-21 (P2)** "Where to spend modeling budget": rank propositions by **each stated range's share of the terminal's band** — how much of the width on the number you care about comes from not being sure of *this* claim — and say so in the UI. On the Hormuz example, 65% of the Brent claim's band is its own `prior`; pin that down and the band goes from 30 points wide to 12. *(Corrected 2026-09-17: read "base rate", and "92% … 29 points wide to 8". The Brent claim has no base rate — the number that varies between versions of the map is its `prior` — and the figures are now the shipped engine's, read on that claim's resolve-by day after the example's window was tuned.)*
