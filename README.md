...
| <http://localhost:8000/docs> | Every route, while the app is running. The three that do the arithmetic — `POST /api/worlds`, `/api/worlds/diff` and `/api/worlds/conditional` — are specified in [`spec/multiverse/`](spec/multiverse/); the one that writes a map from a sentence, `POST /api/generate`, in [`spec/generation/`](spec/generation/); and the three that turn a map into a trade — `POST /api/thesis/card`, `/api/thesis/export` and `/api/thesis/export/markdown` — in [`spec/thesis/`](spec/thesis/) |
