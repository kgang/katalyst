...
Both halves of the drawing below run today, and so does the line between them: **the event stream and the model pipeline are built**. What is still drawn ahead is the grounding adapters and the stored state, and both are named again under it.
...
**The event stream is real, and the browser's half of it is not.** `POST /api/generate` writes `event:` and `data:` lines as a generation happens — a claim proposed, a claim refused, the likelihoods worked through, the verdict, the receipt, and why it stopped — and a terminal with `curl -N` can watch a map being built. **The browser still asks for a finished map and lays it out once**; drawing it claim by claim as the stream arrives is the next pull request, and it is the only part of stack 04 that is not here.
...
**All eight steps run.** A sentence goes in at the top, a map comes out at the bottom, and every step in between is code in this repository. The one thing not here is the browser drawing the map as the events arrive — the stream is served, and `frontend/` still asks for a finished map.
...
```
 ...
 5  api/ emits the event; a reader sees the claim or the refusal immediately    [built, server side]
...
Twelve of the fourteen invariants are fully checked today, and two wait for code that does not exist yet — the thesis (stack 05) and refinement (06). Stack 04 closed the last three partial rows: INV-4's cross-check between the browser's reading of what an edit can reach and the engine's now runs rather than being skipped, INV-8's path product is drawn from a world the engine computed, and INV-13's recorded model answers are committed and replayed on every build.
...
Five of the six rows below exist. Decision record 0008 named four layers and was amended on 2026-09-17 to add the fifth, the worked-example row, which grew out of stack 02. Stack 04 filled the recorded model boundary; the evaluation set is the one row still empty, and it is named here so that adding it is filling a slot rather than inventing one.
...
| Browser app | Component tests for the launchpad, the tiles and belief chips, the wires and their encodings, the layered layout, the diff overlay and delta rail, the branch panel, the inspector and path bar, the keyboard and the outline; plus three checks that read the browser's own source rather than what it draws — no file that draws one of the map's numbers may combine two of them, every colour the law names exists in both themes with its measured contrast, and no file outside the direction readout may name a direction's colour | `frontend/src/**/*.test.{ts,tsx}`, `frontend/e2e/` | `[built]` — **232 browser tests, none skipped** on 2026-09-21, and three end-to-end tests |
| Evaluation set | The four assignment examples run live; structural checks (no loops, ends in a trade, every link has a rationale, verify mode never fabricates a bridge) | `evals/` | `[planned]` — the last empty row. `make eval` says out loud that it is not here rather than pretending. It is the next pull request of this stack, and decision record 0015 leans on it: the scorecard is the baseline any later ensemble would be measured against |

`make test` ran **539 server tests and 232 browser tests** on 2026-09-21 — dated, because the figures move with every round — and `npm run e2e` in `frontend/` (Playwright, the tool that drives a real browser) runs **three** end-to-end tests. None of them needs a key and none of them reaches the network: the end-to-end tests start both halves themselves, and every model answer in the suite comes off disk.
...
Python tests live inside `backend/`, next to the project they test, so `pytest` and `uv` run from one root (decided 2026-09-16). Nothing is created before it holds something, so a directory that is still empty is marked below: `grounding/` holds a package marker and nothing else, `backend/recordings/` is waiting for its first committed generation, and `evals/` does not exist at all.
...
| ADR-0009 | Conventional commits; numbered stacks; PR bases chained by agents, stacks assembled in the GitHub UI | accepted; amended 2026-09-20 (a sixth build job, `recordings`, required on `main`) |
...
| ADR-0014 | A supposition holds until its cause is undermined; the range is how sure we are of the number, computed from two thousand versions of the map | accepted; amended 2026-09-17 (the direction is read with the number's own weights) |
...
Stacks 00, 01 and 02 are merged, **both halves of stack 03 are built**, and **stack 04 is all but its last pull request**.
...
**Stack 04 joined the canvas to the engine and taught the server to write a map.** `App.tsx` hands the canvas an `ApiWorldSource`, with the stored example kept as a fallback that says on screen when it is being used. `engine/` calls the model one proposal at a time, decides every provenance from what the search returned, prices the run, streams eight kinds of event, keeps a transcript in memory, writes a recording and plays one back.
...
- **`[planned]` The map does not draw itself on screen yet.** The server streams a generation; the browser still asks for a finished map and lays it out once. The growing canvas — skeleton tiles, claims as they arrive, wires in causal order, chips last, the refusals beside the map — is the last pull request of 04a and the one thing of this stack not here. There is also no thesis dock, no world-state strip of tradeable instruments and no time axis on which lags are real distances; all three are decided (UX-3) and all three need the stacks after this one.
- **`[planned]` Nothing scores what the model proposes.** `evals/` does not exist; `make eval` says so rather than pretending. Four cases over the assignment's four hypotheses, structure only and never wording, are the next pull request. Decision record 0015 depends on that scorecard existing: it is the baseline against which anything cleverer — an ensemble, a critique pass — would have to prove itself.
- **`[planned]` Two numbers the engine could compute and does not.** Arrow strengths do not yet vary by how well an arrow is backed — the provenance-derived spread decision record 0014 deferred to this stack, which only becomes interesting now that generated arrows genuinely differ. And `domain/` computes no path product: the only multiplied-out likelihood is the Verify door's, worked out in `engine/verify.py` and reported with the caveat that its factors are read on different days.
- **`[planned]` No recording is committed yet.** `backend/recordings/` holds nothing, so a copy with no key can start the app and read the stored example but cannot yet watch a map being built. The four recordings are made by `make record-demo` once the prompt is frozen, and the `recordings` job passes on the empty folder in the meantime.
