...
`make test` runs **526 server tests and 229 browser tests**, and `npm run e2e` in `frontend/` (Playwright, the tool that drives a real browser) runs **three** end-to-end tests. None of them needs a key and none of them reaches the network: the end-to-end tests start both halves themselves, and every model answer in the suite comes off disk.
