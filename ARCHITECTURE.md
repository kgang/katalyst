...
`make test` ran **558 server tests and 240 browser tests** on 2026-09-21 — dated, because the figures move with every round — and `npm run e2e` in `frontend/` (Playwright, the tool that drives a real browser) runs **three** end-to-end tests. None of them needs a key and none of them reaches the network: the end-to-end tests start both halves themselves, and every model answer in the suite comes off disk.
