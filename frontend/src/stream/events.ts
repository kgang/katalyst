 ...
 *
 * **What pins the two together, and when.** A type-level test —
 * `frontend/src/stream/__tests__/eventsMatchSchema.test-d.ts` — fails the build
 * if these eight and the server's ever drift. It cannot be written until the
 * eight are in the generated `frontend/src/api/schema.ts`, and they are not:
 * this half of the product is built against a server being written beside it,
 * and `schema.ts` is generated by the server's own pull request. **It lands in
 * the last of those, with the regenerated schema, and it is the one thing this
 * chapter owes and has not paid.** Until then the guard against drift is the
 * running app: an event name this build does not know is counted and shown
 * (`growth.ts`), which turns a divergence into a number on screen rather than
 * into tiles that quietly never appear.
 *
