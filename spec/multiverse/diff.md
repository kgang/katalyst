...
| `B` | `−.088` | 85.1% | `versions_disagree` | Two large pushes nearly cancel: `S → B` shoves it down and `H → B`'s trigger shoves it back up, and once each version draws its own pushes ([`propagation.md`](propagation.md) B5) about one version in seven comes out the other way |
