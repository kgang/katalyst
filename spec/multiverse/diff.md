...
`N1` going **up** at all is not a slip: `H → N1` is a trigger, H was true on day 0 in every world, so that push fired everywhere and stays fired after the strike withdraws H. Nothing on this branch is `killed`.
...
**Why it had to change, measured.** Hormuz, seed `20261001`, the shipped 2 000 versions × 8 worlds, `observe(B, true)` — *Brent settled below $68*: **209 of the 2 000 versions have no surviving world at all**. Such a version reports nothing to the number and nothing to the band, because its weight is zero — and yet, counted as one vote each, all 209 voted **against** the direction. Counted that way M1 read `87.95%` and M2 `86.70%`, both under the 90% bar, so both read `unchanged`, the rail was empty, and **This happened** looked like a button that does nothing. Counted with the weights, M1 reads `97.16%` and M2 `95.73%` (`98.21%` and `96.82%` among the versions that survived, counted one vote each), both are `shifted`, and both are on the rail. Nothing else moved: under the five edits that are not an observation every weight is 1, so every number this chapter quotes for the strike branch and for `do(H)` is what it was, bit for bit. Worlds per version stays 8 and the 90% bar is untouched.

*(Re-measured 2026-09-21, after the versions began drawing the arrows' pushes as well as the priors. The count of dead versions is the same 209 — it is the observation and the dice that kill a version, not the arrows. The three shares were `89.05%`, `98.66%` and `99.44%`, the same for both endings, when pushes were fixed numbers; they differ between the two endings now because the two endings sit behind different arrows.)*
...
**Such a claim carries `unchanged_because: "versions_disagree"` as well, and the reweighting sentence is the one to show.** Both are true — the half it failed really is the same-direction half — but *nothing agreed with the direction* is a thin way to say *every version that counts moved by exactly nothing*. Two fields, two levels of detail, and the fuller one wins where both are set. Nothing here is a special case in the engine: the words fall out of the two rules as written.
...
**6. Do not read a delta row on the claim's distant resolve-by day.** *Because* two worlds are furthest apart soon after the edit and then drift back together — on this fixture every ending peaks within ten days of the strike, and M1's row reads `−.077` on day 3 against a headline move of `−.042` on its own resolve-by day three weeks later — so a row read there shows a little over half the move and calls it the answer. **Do** read the row at the day of largest divergence, carry that day as `at_day`, and name it on screen.

**7. Do not count the versions one way for the number and another way for the direction.** *Because* a version an observation left with no surviving world reports nothing to the number and nothing to the band — its weight is zero — so letting it cast a full vote on the direction lets a version that contributed to neither answer argue about which way they moved. Measured: 209 of Hormuz's 2 000 versions die under `observe(B, true)`, and counted as one vote each they dragged M1 from `97.16%` to `87.95%` and M2 from `95.73%` to `86.70%`, both under the 90% bar, and emptied the rail. **Do** count a version for the move by as much as it counted for the two numbers, which under every edit but an observation is 1 for every version and changes not a bit.

**8. Do not add a fifth state for a claim moved only by reweighting, and do not add a sixth for the half it failed.** *Because* the four states answer one question — what happened to this claim — and a word that instead answers *how was this number read* puts two unrelated facts under one heading, which is the mistake `killed` nearly made (B2, anti-pattern 3). **Do** leave the state alone, carry `moved_only_by_reweighting` and `unchanged_because` on the claim's row, and let the Inspector turn each into a sentence beside the number.

**10. Do not work out in the browser which half of the `shifted` test a claim failed.** *Because* the floor and the bar are constants inside `domain/diff.py` and reach no reader, so re-deciding at the other end means a second copy of both, re-running the engine's own test against the engine's own answer — and the day the two disagree nobody can say which is right. It is the same mistake as deriving a retraction badge from the branch when the world already carries it. **Do** let the engine say it, in one word, on the row.
