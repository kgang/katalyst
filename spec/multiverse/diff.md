...
**On the Hormuz branch.** `S` is `added`, and every other claim the strike reaches is `shifted`: `H`, `O`, `C`, `B`, `M1`, `M2`, `M3` and `N1`, each read on its own resolve-by day and each on the lines named `… · base · reading` and `… · strike · reading`, with its move on `… · strike · move` and the word itself on `… · strike · what happened`. Every one of those moves clears the `0.005` floor with room to spare — which is what the tests pin, rather than a particular figure. *(Amended 2026-09-22; decision record 0028: there were four before, because `B` and `N1` failed a direction test that no longer exists.)*
...
**On the Hormuz branch.** World A is the base world, so all three edits count and the differing subjects are H (supposed) and S (inserted, then supposed). Every route from either of them to M1, M2 or M3 passes only through `argued` arrows, so the best-backed route's weakest arrow is worth `0.6`. Every route to N1 must end in `H → N1`, which is the only arrow into N1 and is `asserted`, so the bottleneck is `0.3` however well-backed the rest of the route is.

The rail the engine actually produces, base world against the strike branch, is four rows. Each row's place, its day, its two readings, its biggest move and its rank are lines of their own in the numbers file, named `<ending> · strike · change list place`, `… day`, `… was → is`, `… biggest move` and `… rank`. The order, which is what this passage is about:

| Place | Ending | Why it sits there |
|---|---|---|
| 1 | **M3**, the Polymarket contract on the strait's own traffic | the largest move on the map by a long way, along `argued` arrows |
| 2 | **N1**, the talks nobody quotes | a large move, discounted by the one `asserted` arrow that is the only way into it |
| 3 | **M1**, the Brent contract | a small move, well backed |
| 4 | **M2**, energy shares against the market | a smaller move, equally well backed |

**N1 moves further than either Brent ending and still ranks below M3**, which is the second factor earning its place. The only way into N1 is `H → N1` — an arrow whose own rationale admits it cannot say which way the causality runs, since quiet talks may be what reopened the lane rather than the other way about. That is written on the arrow as `asserted`, worth `0.3` against the `0.6` behind every market ending, so the row is discounted to well under half of what its size alone would buy.

**That is the right answer and it is worth saying why.** The change list is for changes a reader can act on, and *this moved a long way along the one arrow nobody could back* is a change to read after the ones that did not. The move is not hidden — it is on the row, with its size — it is ranked where the quality of its evidence puts it.

*(Amended 2026-09-22; decision record 0028. Until the versions were cut, `asserted` was read **twice**: once by this ranking, and once by the engine itself, which drew that arrow's number widest of all so that fewer than 90% of the versions agreed on the direction and N1 fell off the rail entirely, reading `unchanged`. It is now read once, here — and N1 is on the rail, second, rather than missing from it. The conclusion a reader draws is the same one and it is reached in one place instead of two.)*

**And the rail under an observation**, base world against `observe(B, true)` — *Brent settled below $68* — which is what **This happened** produces on the same map. Every ending rises, because cheap Brent is evidence for all of them, and the order is M1, M2, M3, N1: the lines named `<ending> · observed B · change list place` say so, with each row's day and readings beside them.

**N1 is on that rail too, last.** It is reachable — the observation climbs from B to H and runs forward down `H → N1` — and it clears the floor, so it gets a row. Until the versions were cut it did not: its share of versions agreeing on the direction was `42%`, well under the ninety-per-cent bar, and both market rows read `88.6%` and `87.4%` when every version was counted as one vote, so on that reading the whole table was empty. That was open question 7. **There is no bar and no share now** (decision record 0028), and a row needs a move of `.005` and nothing else.

**The Inspector's path bar walks this same route** and names its steps *(renamed 2026-09-22: it no longer shows a product — decision record 0022, and B5)*. One path-choosing rule, used twice, so the bar and the rail can never point at two different chains through the same map. When two routes are equally well-backed the **shorter** one is taken, and if they are still level, the one whose first differing arrow comes earlier in the map's own list of arrows *(proposed here)* — the rank does not care, because tied routes give the same weight, but the bar names its steps, so the choice has to be the same every time. On the Hormuz map every arrow into M1 is `argued`, so `H → B → M1` wins over `H → C → B → M1` by being shorter.
...
| A delta rail row | The **day of largest divergence** between the two worlds | Two worlds pull apart at different rates and are not furthest apart on the day the claim is judged. The row carries `at_day` and `peak_delta`, and names the date on screen |

**Why the second row is not pedantry**, read off the fixture as it stands. **M3 is the case that shows it**: the line named `M3 · strike · change list day` is 2026-10-27, five days before M3's own resolve-by day, and `M3 · strike · change list biggest move` is larger than `M3 · strike · move`, which is the headline move read on the judged day. The two worlds pull apart, and then the branch's curve runs up against certainty and the gap closes a little.

**On three of this fixture's four endings the peak now falls on the judged day**, and that is a change worth stating rather than hiding *(measured 2026-09-22 at the flip)*. An event's series can only rise, so two rising curves usually go on separating until one of them saturates; where neither saturates inside the window, the largest gap is at the end. So the day this row names is often the claim's own judged day and sometimes is not, and the rule stays as it is: read the row where the two worlds are furthest apart, which is a fact about the pair of curves rather than a fact about the calendar. Anti-pattern 6 below is unchanged for the same reason — the *day* still has to be named on screen, because a reader cannot tell which of the two kinds of case they are looking at.

**Why the two worlds' curves cross and separate** *(restated 2026-09-22; decision records 0016 and 0017)*. This passage used to say that a change *shows up for a fortnight and then unwinds*, and it was written for a likelihood read on one day, which could rise and fall freely. **An event's series can now only rise**: it is the chance the claim has happened by that day, and a thing that has happened does not un-happen. So the gap between two worlds' curves no longer unwinds of its own accord — it opens where one world's claim is arriving faster than the other's and closes again as both approach certainty or neither does. That is still a curve with a peak somewhere other than the claim's own judged day, which is all the rail's day was ever for. **A falling curve returns with states**: a state's series is the chance it is still holding, so two worlds' state curves can genuinely unwind, and the old sentence is true of a state and of nothing else — on this fixture, of O and of C.

**A second known wart: a row's day and its rank are read off the *drawn* series, and an unrelated edit can move them** *(added 2026-09-21, re-measured the same day)*. `at_day` is the largest divergence over the days **both worlds drew**, and past 180 days those days are spread evenly over the window ([`propagation.md`](propagation.md) B1). The window runs to the last day anything is judged, so an `insert` anywhere on the map — in a piece nothing connects to this claim — re-spaces them. `before`, `after`, `range_width` and `agreement` are all read on `at_day`, and `rank` is `|peak_delta|` times a weight, so **every number in the row follows the day**.

Measured on the shipped fixture, the strike branch against the base world, with one further claim inserted and judged 365, 900 and 1 825 days out — a claim with a push of nothing, in a part of the map that reaches neither ending. Of the fourteen fields the rail's two rows carry beside their names, **seven move in at least one of the three runs**: M1's day, its two likelihoods, its `peak_delta`, its `range_width` and its `rank`, and M2's day. At 1 825 days the two rows even **swap order**, because both peaks then land on the same drawn day and M2 ranks above M1 there. Nothing about the arithmetic moved — see below — and the rail has read the drawn series since it was written, so this is not something the thinning introduced.

*(This passage used to quote a two-piece map moving an ending by `+.27` on day 3 and `+.21` on day 4, and "22 fields" on the Hormuz rail. Neither could be reproduced on 2026-09-21 from the construction described here, on this engine or on the one before the arrows' pushes varied — the two-piece map gives `+.033` on both windows. The measurement above replaces them and says its own method, so anybody can re-run it.)*
...
**And it cannot be fixed by choosing better days.** The divergence between two worlds is a continuous curve; its peak can fall anywhere between two drawn days, so finding it truly would mean drawing **every** day of the window. **What the rail promises is the largest divergence among the days it drew, not the largest there is.**

**The wart that used to sit here has gone with the number it was about** *(2026-09-22; decision record 0022)*. INV-8 required a path's multiplied-out likelihood beside the headline, and this paragraph admitted that its factors were each read on a different day, so it was a product across days rather than a likelihood at one instant. **Multiplying along a route is not the chance of anything** once two claims on the map share a cause, so the number is deleted rather than annotated. Three worked-out quantities replace it — the shift the hypothesis makes to its destination, the chance every step on the named route goes right as a true joint, and which single arrow carries most of the shift — each computed from the world by the exact solve. The route they are read along is B4's, unchanged.
...
**And a third, when an ending was forced false** *(added 2026-09-22; the third of the three wordings)*. `killed` means an edit assigned the claim false, and a `killed` ending never reaches the rail, because the rail holds `shifted` rows. So the old second sentence printed **"moves no ending and leaves 6 claims untouched"** beside an ending the reader had just struck off the map, which is the most misleading sentence this product can write. When any terminal came out `killed`, the summary names it instead:
...
This chapter uses the local numbers `INV-multiverse.18` through `.34`. `.1`–`.5` belong to [`interventions.md`](interventions.md), `.6`–`.8` to [`branches-and-worlds.md`](branches-and-worlds.md), and `.9`–`.17` to [`propagation.md`](propagation.md).
...
Raised 2026-09-17. The first four were settled the same day and their answers are recorded in place below. The seventh was raised by the engine itself, settled the same day, and closed for good on 2026-09-22 when the mechanism behind it was deleted. **The fifth and sixth stay open and name who owns them.**
...
6. **Diffing more than two worlds.** `diff` takes exactly two worlds. Three pairwise diffs against a common base world is the obvious reading and nothing here forbids it, but nothing names it either, and the delta rail's ranking across three lists is undesigned. `spec/workbench/` owns how several branches are compared on screen. **Much less pressing since 2026-09-21** (decision record 0021): FR-17, which asked for three or more branches side by side, is not built in version one — three branches at once is a dashboard, and the ranked change list already answers the question two worlds at a time. The question stays open because the design gap is real, not because anything is waiting on it.

7. **An observation rarely puts a row on the rail.** *Raised 2026-09-17, when the engine first ran it; answered the same day, and the answer is now history.*
   **Closed 2026-09-22 by decision record 0016.** The cause was that *This happened* threw draws away, so hundreds of versions could be left with nothing and were voting anyway. Kent's answer (G2) was to count each version by how much it survived; the exact core removes the discarding, so no version is ever left with nothing and the question cannot be asked again. **What the two answers had in common is what matters: an observation must be able to put a row on the rail, and on this fixture both tradeable endings come out `shifted` under *Brent settled below $68*.** B3 keeps the measured history of the weighted answer, and the dated amendment to record 0014 keeps the decision.
