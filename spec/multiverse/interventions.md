...
| `retune` | Button **Change this push**; badge **Retuned**, read back as "you moved this arrow from +0.7 to +0.3" | Disagree with one number on one arrow | *Structure:* the `strength` field of one link. *State:* everything downstream of that link | Every other field of that link — mode, lag, shape, half-life, rationale, sources, provenance, and which two claims it joins. Every other link. Every proposition. Every belief |
...
**And the override is visible (UX-14).** A claim that was supposed true and has since been pushed back down is never drawn as plainly true. H's tile reads **Supposed · Oct 1 → Retracted · Oct 2 by "confirmed strike on Iranian territory"**, naming the edit responsible, and the branch panel lists the three edits in the order they were made. The rule above is surprising if you meet it in a number and obvious if you can read the sequence, so the sequence is on the screen.
