...
Every arrow is one of two kinds, and an intervention treats them differently *(rewritten 2026-09-22; decision record 0017)*. A **trigger** link is a domino: it reads the day its cause came on and keeps pushing afterwards, whatever the cause does — standing the earlier domino back up does not stand the later one back up. A **sustain** link is a desk holding an apple: it reads its cause's **whole stretch** and is dead the moment the cause stops holding.

Because only a claim that can stop holding has such a stretch, **a `sustain` arrow may leave only a state** — a claim that holds over time rather than happening once. An **event** happens once and stays happened, so nothing sustained by an event ever loses its desk. That rule is refused by name, `sustain_without_state`, **from stack 05's shape freeze**, which is also when the model is first asked which kind of truth each claim is.

In the Hormuz branch, `do(S, true)` makes the strike true on 2026-10-02, and three arrows out of S go live. **Nothing is withdrawn and nothing retracts**: both supposed events stand, and what the strike ends is the **state** they were holding up — *the strait stays open to commercial transit through 1 November* — which takes down every claim that state sustains. H → B, by contrast, is a trigger that already fired: that domino stays fallen while S → B pushes B the other way. Which arrows stay live is settled here; by how much each one moves a number is [`propagation.md`](propagation.md)'s business.
...
**What goes with the rule.** The derived badge *Retracted · date · by "…"* — the one badge no button produced — and the two day-states `withdrawn` and `pushed`. A claim's day is now `sampled` or `supposed`, and nothing else. UX-14 keeps its job in a different shape: **a claim that stops holding says when, and what stopped it**, which is a sentence about a state and never about a supposition, and the branch panel still lists that branch's edits in the order they were made. The rule that a later `insert` outranks an earlier assertion still holds in the patch algebra and is still readable in that ordered list; there is simply no longer a badge for it to report.

**INV-3 is untouched by this.** `do(H)` still moves nothing upstream of H; its own reach is H and what H causes. Everything the strike moves belongs to the two edits that produced it — `Insert(S, …)` and `Do(S, …)` — whose affected set includes what S now reaches. Every claim that moved can still name the edit that moved it, which is the property the whole branch mechanism exists to keep.

**The other half of the showcase — H → B.** H → B is a **trigger**: it fired on 2026-10-01, B fell, and that domino stays fallen. The spike fades on its own half-life from then on, while S → B pushes B the other way. One branch, both kinds of causality — the sustaining kind and the sequential kind — which is exactly the distinction in `docs/initial-brainstorming.md`.
...
An observation is not a number stored once. It is a condition re-read through the map every time the map changes: every number the observation touches is worked out with the observed claim's value held to what was seen. So changing anything that can move the observed claim changes what the report says about all of them. *(Amended 2026-09-22: this used to describe a filter keeping only the draws that agreed with the news. The exact core conditions instead of discarding — [`propagation.md`](propagation.md) B6 — and the reach of an observation is exactly what it always was.)*
...
* **Withdrawing the news.** Suppose the very claim that was reported. A later edit on a claim overrides an earlier one, so the observation stops holding, its condition is lifted, and everything it was evidence about moves again. **The reach of a standing observation is therefore read off the map the edit started from**, not the one it left behind: a `do` cuts the arrows into its target, so on the map afterwards the causes it has just stopped speaking about are no longer even connected to it.

A `believe` never triggers it, and that is the reason it has its own row: the user's number is not pushed through the map, so it cannot change what an observation is evidence about.
...
1. **Does `do` cut an arrow inserted after it?** This chapter says `do` cuts the arrows that exist *when it is applied*, so an arrow inserted afterwards is live and can act (B2). The alternative reading — that `do` seals its target against every later incoming arrow — kills the showcase. Confirm the first reading, and `propagation.md` (stack 03a) encodes it.
   ...
   **Extended 2026-09-17 (decision record 0014), and reversed 2026-09-21 (decision record 0017).** Record 0014 settled *which day* a supposition ended on — the day the cause of the first live opposing arrow became true — with three named day-states. **Nothing ends a supposition now but another edit.** What the live arrow ends is the **state** the supposition was holding up, and while a supposition holds the claim is true in every version and the tile shows the word rather than a number, which is the half of record 0014 that stands. See B2.
...
7. **Interface wording for the first two verbs.** Decision record 0004 calls them "assume" and "learn"; this chapter writes **Suppose this is true** and **This happened**, which read better on a box. `spec/workbench/` owns the final copy; whichever wins, `spec/vocabulary.md` should carry it.
   **Decided 2026-09-17:** this chapter's pair wins, for all six operations and their badges, and `spec/vocabulary.md` now carries the full list under *Interface words*. The two headline verbs are **Suppose this is true** (badge **Supposed · date**) and **This happened** (badge **Happened · date**). Record 0004's "assume" and "learn" are corrected in place, dated. *(Amended 2026-09-22; decision record 0017.)* That list used to carry one badge with no button behind it — **Retracted · date · by "…"**, derived rather than produced. Nothing derives it any more, because nothing retracts itself, and it is deleted along with the two day-states it was drawn beside. **Every badge this product draws is now produced by a button somebody pressed.**
