...
4. **Do not say "confidence" when you mean probability.** Because the vocabulary reserves *confidence* for something else — how sure the model is about its own reasoning, and, on a link, how well-founded the mechanism is (`speculative` / `argued` / `documented`, see [`link.md`](link.md)). Two different ideas under one word makes both unreadable. **Instead:** say *likelihood* or *probability* for the number, *range* for `lo`–`hi`, and *confidence* only for the link field.
