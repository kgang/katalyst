...
The reason is that **the guard's words have to mean what they say.** `<.01` is a sentence, not a shrug: a chip showing it beside a likelihood of `.0035` is telling the reader something true, and one showing it beside `.010` would not be. The same holds at the top. So the boundary is not chosen separately from the words — it is read off them.

Two things follow rather than being decided beside it:

* **Both guards' own numbers print.** `.010` is not below `.01` and `.99` is not above `.99`, so each prints its figures. `.0099` is below, and is guarded; `.995` rounds to one, and is guarded.
* **The figures always land in the first two places after the point**, `.99` down to `.010`. Anything further down rounds to less than a hundredth and anything further up rounds to one, so those two places are the whole of the printable range.

**A move is not a likelihood, and keeps its two figures however small it is.** The stored example has one to hand: observing that the war-risk premium printed below 0.4% nudges the strait-open claim by less than a hundredth, and that move is still written with both of its figures. The claim's reading and the move itself are the lines named `H · observed C · reading` and `H · observed C · move` in [`docs/worked-numbers.txt`](../../docs/worked-numbers.txt) — the one generated file that owns every number this example's engine works out, written by `make numbers` and checked by the build, and the only place either figure is kept. A move that small is a real quantity a reader acts on, while a likelihood that small is one this product declines to state that precisely and prints as `<.01`; putting both under one guard would hide a change the reader asked to see. Whatever writes a move writes it with its own formatter, never this one.

The worked table of awkward cases is in [`../workbench/keyboard-and-access.md`](../workbench/keyboard-and-access.md) B6, which owns the rendering rule for the whole part. **The rule is implemented twice** — `two_figures` in `backend/src/katalyst/domain/belief.py`, which every part of the server writes a likelihood through, and `toTwoFigures` in `frontend/src/components/BeliefChip.tsx`, which writes every chip. They are one rule and must move together; where the two stacks meet, the same cases are run through both and required to agree, because one number reading two ways on one screen is the failure this is guarding against.
