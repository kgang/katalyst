...
**A move is not a likelihood, and keeps its two figures however small it is** — `.36 · up by .0090`. A move of `.0090` is a real quantity a reader acts on, while a likelihood of `.0090` is one this product declines to state that precisely; putting both under one guard would hide a change the reader asked to see. Whatever writes a move writes it with its own formatter, never this one.

The worked table of awkward cases is in [`../workbench/keyboard-and-access.md`](../workbench/keyboard-and-access.md) B6, which owns the rendering rule for the whole part. **The rule is implemented twice** — `_two_figures` in `backend/src/katalyst/domain/diff.py`, which writes the change list's sentence, and `toTwoFigures` in `frontend/src/components/BeliefChip.tsx`, which writes every chip. They are one rule and must move together; where the two stacks meet, the same cases are run through both and required to agree, because one number reading two ways on one screen is the failure this is guarding against.
