...
Beliefs are rendered at **two significant figures with the range** — the number and both ends — `.35 (.22–.50)`, never `.347`, and never as a certainty: what would round to `1.0` prints `>.99`, what would round to `.0` prints `<.01`. Precision beyond two figures on an elicited number is a lie about how much we know — the honesty requirement, NFR-1 in `PRODUCT_REQUIREMENTS.md` §8.
