...
Between those anchors the interface reads a strength back in **five bands** — *a faint push* below 0.25, then *a nudge*, *a clear push*, *a strong push*, and *close to decisive* (or *close to ruled out*) — which are these anchors with the gaps between them filled. The band edges are in [`../workbench/tiles-ports-wires.md`](../workbench/tiles-ports-wires.md) (decided 2026-09-17).
...
**`provenance` says where this link and its number came from.** It is a fact about our pipeline, recorded by `engine/` from what actually happened during generation; the model never declares it (decision record 0003, rule 5). It is what the canvas encodes as a three-step origin mark at the wire's tail — the stroke itself says what kind of push the arrow carries, and nothing else — so a user can see at a glance which arrows have documents behind them and which are the model talking (INV-2, INV-12).
...
The user clicks the wire from `H` to `B` and the Inspector shows the rationale, the strength as "+1.6 — a strong push toward", and a two-dot origin mark beside the word *argued* — the model gave a mechanism, and no document stands behind it. They can `retune` the strength if they disagree ([`../multiverse/interventions.md`](../multiverse/interventions.md)).
...
On the canvas the wire carries a three-dot origin mark at its tail — there is a document behind this arrow — and the Inspector lists the source with its title and the date we fetched it.
...
4. **`half_life` on a non-`impulse` link.** *Decided 2026-09-17: rejected as a violation (`half_life_without_impulse`); reject, never repair.* It is the thirteenth code, checked by `validate` rather than by a field validator, because a model proposal gets the shape and the half-life out of step often enough that the user should read a sentence rather than a stack trace. **The other direction — decided 2026-09-17: also a violation, `impulse_without_half_life`, checked by `validate`.** A spike that fades must say how fast; the shape and its numbers must agree, checked both ways, so working the likelihoods through never has to invent a default. *As first asked:* an `impulse` that names no half-life is legal today and the push then has no stated decay. Does it take a default, and if so whose — a number on the link, a constant in propagation, or a re-prompt? Settle before propagation is written in stack 03a.
5. **Does `ramp` need its own duration?** *Decided 2026-09-17: no. `lag` is a ramp's rise time and there is no `rise_time` field in version 1; `spec/multiverse/propagation.md` writes the shape out as arithmetic.* This chapter defines all three shapes as reaching full size at the end of `lag`, with `ramp` climbing across that window. That makes `lag` do two jobs for `ramp` — dead time for the others, build-up time for it. The alternative is a separate `rise_time` field. Settle before propagation is written in stack 03a.
6. **Should `strength` have a sanity ceiling?** *Decided 2026-09-17: no ceiling. `validate` is untouched; working the likelihoods through adds a plain warning to the world when an arrow's strength is beyond ±5 (`spec/multiverse/propagation.md`).* It is an unbounded float. A model that writes ±12 has effectively asserted certainty while looking like it gave a number. A soft cap of about ±5 (roughly 1% to 99% on a coin flip) would catch that, at the cost of a rule with no principle behind it.
