...
**Rule 13's three numbers are written the way the product writes a likelihood** *(2026-09-21)*. This message reaches the browser inside a refused request, and it used to carry whatever precision the computer happened to hold a number at — a tenth plus two tenths arriving as `0.30000000000000004`. `<p>`, `<lo>` and `<hi>` are now two significant figures, the same rule the chips on the canvas obey. A number that is **not** between 0 and 1 reads *outside 0 to 1* in words instead of going through that rule: the rule answers `>.99` for 1.4, and that word inside a sentence saying the number is out of range would tell the reader the opposite of the truth. The rule lives on `Belief` itself — see [`belief.md`](belief.md) — so the two places that write a belief into a sentence cannot drift.
