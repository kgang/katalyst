...
**The word we stamp here says where a number came from, and sets nothing else** *(rewritten 2026-09-22; decision record 0028)*. It is shown on the arrow, it decides whether a proposal is allowed to claim evidence, and the change list reads it when it ranks a move by the weakest backing on the best-backed route. It moves no number.

> **What this paragraph used to say.** *The word we stamp here is also how wide that arrow's push gets drawn* — the engine ran two thousand versions of the map, each drawing every arrow's push from a bell curve centred on what the map states, and how wide that curve was was read off this one word, narrowest for the three that mean somebody fetched something and widest for a bare assertion. **There are no versions and no range** (decision record 0028, Kent's row R48): an arrow's push is its stated strength, used once, and the width had nothing left to make differ. One thing that paragraph argued survives it and is still true — **nobody is asked a second question**, because *how well-backed is this* and *how sure are we of this number* were the same question, and asking twice would have invited two answers that disagree. **No arrow this pipeline produces is a bare assertion**, either: the map's rules refuse an arrow with no mechanism at all, and the table above says a proposal can only come out `documented` or `argued`.
