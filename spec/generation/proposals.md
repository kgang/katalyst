...
**The order answers are folded in is the order the calls came back in** (*Kent, 2026-09-22*). A round's three calls finish minutes apart — on the run Kent kept they took 131, 44 and 65 seconds — and folding in the order the questions went out holds the 44-second answer behind the 131-second one, so all three claims appear together and the screen is dead for the 87 seconds after the first answer already existed. Each answer is folded, and its event sent, the moment its own call returns.

**What that costs, said plainly.** Which of two answers is the one refused, the order of the frontier, and therefore which claims the next round asks about, now depend on which call came back first as well as on the map and the answers. It is paid on purpose, because nothing that has to be exact rests on it: every answer is judged again at the fold, so the map that comes out is one the rules accept whichever order they land in; identifiers are minted inside the call rather than at the fold, so they do not move; and a recording holds whatever order actually happened, so a replay of one is exact either way. One consequence reaches the screen — a round that closes two lines at once names whichever of them landed second as the reason the run stopped (B4) — and both sentences are true of the run, each naming the line the reader watched close last.
...
The stream ends with one `done` event carrying **one** of seven reasons. **The rule: the reason names what closed the last claim that was still open**, with two overrides above it. One rule, so two readers of one run cannot get two answers. *"Last" means last as the reader watched it close* — which since 2026-09-22 is the order the calls came back in — so a round that closes two lines at once names whichever of them landed second, and the very same answers arriving in the other order would name the other. Both are true of the run that happened; nothing in production asks the same answers twice, because they come from a model and a replay plays a recording back rather than walking again.
...
One Hormuz generation through the Explore door, at the default caps: depth 5, width 3, 30 claims. `at` is the position in the transcript, from 0. The browser chapter draws this exact run, so nothing here is approximate — it is one run that happened, and steps asked in the same round are numbered in the order those calls came back, which is what the reader watched.
...
**What a ceiling really promises.** A round of up to three questions goes out together, and **each is billed the moment its own call comes back, before that answer is folded** — so a bug in the folding can never take a paid call off the receipt, and whatever the round still has in flight when one throws, or when the reader walks away, is waited for, billed and folded where nobody sees it (*Kent, 2026-09-21; kept this way rather than by billing the whole round up front when answers began arriving one at a time, 2026-09-22*). The other calls of a round are still out when the ceiling is reached, so "checked after every call" overstates it. The honest bound: **a run can pass its ceiling by at most the calls that were in flight when it was reached, plus the rounds of research one of those calls may still run.** Nothing further is *asked* once it is reached, which is the part that matters — including the scripted intervention, which a run that ended for money does not draft at all (Kent, 2026-09-21).
...
### B9 — the stated likelihood ships as stated, and the range beside it is read by nothing
...
The one-sentence reason, with the record to read for the rest: decision record 0014 defined `lo` and `hi` as the 10th and 90th percentiles of **the likelihood itself**, and a spread measured across re-asks is a different quantity — how much one model wobbles when asked the same question twice — so one field would be carrying two meanings. **Decision record 0015** has the other three reasons, the known cost said honestly, and what would reopen it.

**The two ends are asked for and nothing reads them** *(amended 2026-09-22; decision record 0028, Kent's row R48: one likelihood per claim, computed once, and no range anywhere on this product)*. Only the model's `p` reaches a belief. The heading of this section read *the stated range ships as stated, and is labelled*, and the paragraph under it said the chip keeps record 0014's label — *model interval, uncalibrated* — with the two sentences the workbench owned; there is no such chip and no such label, because there is no range on screen to label. Record 0015 is **not** reversed and the model is still asked for all three numbers, for one reason: what the model is asked for is part of the prompt's fingerprint, and changing it invalidates every recorded answer this repository replays. Whether to stop asking rides stack 05's one shape freeze.

The honest cost the label used to carry is still worth stating here, where the numbers arrive: a likelihood a language model states is reliably overconfident, and the figure always travels with its source (FermiEval: a nominal 90% range covered the truth 28% of the time, cited in record 0014's research note). Nothing in this product claims its likelihoods are calibrated.
...
9. **Do not widen the stated range because the research says stated ranges are too narrow** — *because* the field would then hold a number nobody stated and nobody measured. **Instead:** store what was stated, unchanged, and quote the coverage figure with its source when the narrowness comes up. *(Amended 2026-09-22; decision record 0028. The instruction used to end "label it* uncalibrated*"; there is no label, because no range reaches a reader. The rule still binds what is stored: a number nobody stated is a number nobody can trace.)*
