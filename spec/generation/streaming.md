...
**Both fields are accepted and ignored since 2026-09-22** *(decision record 0028, Kent's row R48: one likelihood per claim, computed once, and no range anywhere)*. The engine works a map out once, so there is nothing for either number to size. They stay on the wire until one later pull request takes them off, and **the bounds below still apply to them**, because a field a caller can send is a field a caller can send a hundred million in. Everything this section says about how a number out of range is answered is unchanged; what has changed is that the number inside the range no longer decides anything. The paragraphs below are kept as they were written, because they are why the bounds exist.
...
Local numbers in `spec/generation/` come from one shared pool. **This chapter holds `INV-generation.16` through `INV-generation.21`, and `INV-generation.33`**; the split across all five chapters is on the [landing page](README.md). *(The block is not one run because the chapter gained an invariant after the pool had been handed out: `.22` was written here on 2026-09-22 and was already [`replay.md`](replay.md)'s, so it was corrected the same day to `.33`, the next number free after the last chapter's `.32`. A number means one thing; the cheap fix was the wrong one.)*
