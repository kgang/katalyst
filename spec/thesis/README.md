...
**The honest refusals are load-bearing.** A read of the venue on 2026-09-21 found nothing quoting Brent crude, and that the one Hormuz contract it does quote resolved on a different test from the claim on the map. Kent's decision R30 settled that, and it landed on 2026-09-22. The curated map's hypothesis now asks the venue's own test word for word, and a new ending — *a Polymarket contract "Strait of Hormuz traffic returns to normal by October 31?" resolves YES* — names that contract and that side, types no price at all, and takes its market voice from the committed quote file. So the **curated** example carries exactly **one** real venue price: its Brent ending keeps a stand-in contract and a stand-in market number, both labelled, because nothing on the venue quotes Brent; and its diplomatic ending stays not-tradeable, now for the true reason — the contracts that exist ask about any mediated United States-Iran round rather than about an Omani-mediated one both governments confirm. The **recorded** map the walk opens has eleven endings naming instruments and no contract, so it prints *no contract quotes this claim — edge not calculable* eleven times, honestly, until the one paid re-recording after the freeze.
