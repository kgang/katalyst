...
**Every refusal is a list, and every list arrives at once.** A branch that does not fit the map and a form the reader cannot be given both come back as `422` carrying every reason together, each with a stable code a screen can switch on, the thing at fault — a claim, an arrow, or the name of a form field as the reader sees it — and one plain sentence. A name matching no stored example is a `404` naming the ones that exist. Nothing is repaired silently and nothing arrives one fault at a time.
