...
The **greyed ceiling** carries its label *never size to this* as a field on the value; it is a quartered Kelly at the unfavourable end of the model's stated range, reads **zero with a reason** where that range does not agree which side of the price to be, and is **absent with the refusal's sentence** wherever no edge can be built (Kent, R29). Zero and absent are different answers and the card says which. *(Amended 2026-09-22; decision record 0028. There is no range: the unfavourable end of it is now the likelihood itself, and the zero-with-a-reason case cannot arise from anything the engine computes, because it is the straddle below. Both rules are kept as written.)*
...
*(Amended 2026-09-22; decision record 0028, one likelihood per claim and no range anywhere. The first of the two thresholds no longer reaches this card from the engine: an edge's `inside_the_model_range` is always false, because both ends of every range are now the same number. **The card's two refusals are kept exactly as written above** — it neither headlines nor ranks such an edge — so the rule stands ready if a range with width ever returns, and two tests set the flag by hand to keep those refusals checked. The one threshold that bites today is the venue's smallest price step.)*
...
**One sampler, and it is the engine's.** The drawn worlds come from the engine's own sampler, asked for the same map, branch and seed the world beside them was built from, and corrected with the finished answers that world carries rather than with the uncorrected ones the solve produces before a report is folded in. A route that assembled a second draw of its own would be a second chance for the card and the tiles beside it to disagree about one map, and — under *This happened*, where the two sets of answers are different numbers — a chance to correct against the wrong ones. How many worlds to draw is a field on the request, bounded by the count the engine draws for itself: a request above the ceiling is refused rather than quietly made smaller. And a *This happened* edit **nothing on the map agrees with** leaves no weighted world at all; the engine says so by name and the route answers `nothing_agrees_with_what_happened` on the field `branch`, rather than a server error.
