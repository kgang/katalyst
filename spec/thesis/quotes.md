...
* **resting size and traded volume**, because a midpoint over an empty book is a different claim from one over a hundred thousand dollars of resting orders; and **all four state flags** — active, closed, archived, accepting orders — because a settled market keeps serving a book and live-or-settled is read from these, never from the price. *(Corrected 2026-09-22: this said two flags. Of the 1 693 real market records saved for this chapter, **226** say they are open and taking orders while also saying they are not active, and one of those says it is archived too. Two flags call all 226 tradeable.)*

* **What only a venue can tell us, and what a venue always tells us, are two different lists.** A price the reader typed carries none of the venue's facts. A price read from a venue carries the twelve that every one of those 1 693 records has — the venue, its three identifiers, the side priced, its own question and rules, a web address and the four flags — and may be missing any of the five it sometimes leaves out: 154 carry no resting size, 189 no traded volume, 24 no end date. An absent one stays **absent**, never nought, and a parser that demands them refuses working markets. *(Added 2026-09-22, after a sweep of the saved answers found the parser crashing on 171 of them.)*
...
**INV-thesis.4 — No price arrives over a network in a test.** Over every test module in `backend/tests/grounding/` and `backend/tests/thesis/` — the two directories that touch a price — every test runs with the machinery for opening a connection replaced by something that raises, and the suite passes: the recorded loader reads the committed file, and the live adapter is exercised only against saved responses. *(Narrowed 2026-09-22 from "every module in `backend/tests/`". The guard is opt-in on purpose: the tests at the model boundary replay recorded exchanges through a library that intercepts connections itself, and two guards fighting over the same machinery would tell a confusing story about which one refused.)* **Tests:** `test_the_guard_really_refuses_a_connection`, `test_no_quote_test_touches_the_network`, `test_the_best_bid_is_not_the_first_bid`, `test_a_settled_market_is_read_from_its_flags`, `test_every_saved_market_record_reads_or_says_why`.
...
2. **Do not read the price off the venue's display figure, or off the first line of the book.** Because the display figure is rounded and snaps to one and zero on a settled market, and bids come back ascending and offers descending, so the first is the *worst* — in the response saved for this chapter, `.01` against a best bid of `.06`. **Instead:** take the **best of each side by value** — the largest bid and the smallest offer, which is what *best* means and does not depend on the venue's ordering staying what it is today — derive the midpoint, and keep a saved response where the best and the first differ as a test. *(Built 2026-09-21: the first draft said "take the last of each side", which is the same answer on today's ordering and a wrong one the day it changes. The test that checks it shuffles the saved book's two sides and demands the same answer, because a test that reads the last line cannot tell the two rules apart either.)*

3. **Do not decide a market is live from its price**, because a settled market keeps serving a plausible book: read the venue's flags — **all four**, since 226 of the 1 693 real records saved here say they are open and taking orders while also saying they are not active.
