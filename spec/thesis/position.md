...
**Where the days come from.** The engine's own **weighted forward sample**: worlds drawn forward carrying event days, with an observed claim set to what was observed and the world weighted by how likely that was — one sampler, two readers. **So the weight is real, not all ones**, and every count here is weighted. Record 0019 carries the measurement, the script and the caveats; every first-touch number on screen names the sample it rests on.
...
Given a risk budget and the distance from entry to stop, the size that loses exactly that budget is arithmetic on two numbers the reader typed; the card shows it as what their own rule implies, **never as a recommendation**. Beside it sits a greyed **quartered-Kelly ceiling** labelled *never size to this*, defined in `card-and-export.md`. On getting out the card says one thing and no more: *a stop order's trigger is not its fill price.*
...
**INV-thesis.16 — A path invents no advantage.** For every position from `positions()` over every set of draws from `draws()`, with each claim's market chance taken from the base world's own number for it, so nothing is typed in: the mean move over the window is zero within the sample's own error, and the move applied when a claim comes true plus the giveback while it has not sum to its stated move. **Tests:** `test_a_path_has_no_drift_when_the_model_agrees_with_the_market`, `test_the_applied_move_and_the_giveback_sum_to_the_stated_move`.
