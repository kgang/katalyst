...
`inside_the_model_range` — *whether an edge is worth taking at one end of the model's own range and not the other* — is **always false** *(amended 2026-09-22; decision record 0028, one likelihood per claim and no range anywhere)*. Every number the engine computes now has the same value at both ends of its range, so no sign can change between them, and `edge.py` sets the field to that constant. **The rule is not withdrawn:** every reader of the flag still refuses to headline, rank or size such an edge, so that a range with width returning from somewhere — a reader's own stated uncertainty is the obvious candidate — finds the refusals already written down. Tests set the flag by hand to check those readers.
...
The reader selects an ending a venue quotes on the curated Hormuz map — the one ending there is, added by Kent's decision R30, which names the real Polymarket contract on the map's own hypothesis and takes that contract's resolution test as the claim's. The card shows the model's number, the venue's bid and offer with their date, the fee, and the better of the two edges. Two things stop it being headlined, and the card says which bit: a gap narrower than one **tick** — the smallest price step the venue trades in, one cent on its Hormuz market — and a gap whose sign changes between the two ends of the model's own stated range. *(Amended 2026-09-22; decision record 0028. The second of the two can no longer happen: there is no range, so both its ends are the same number. One tick is the only threshold that bites today, and the card's refusal to lead with the other is kept for the day a range with width comes back.)*
