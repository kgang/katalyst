...
| **INV-workbench.23** | For every map and every re-layout of it, no already-placed tile changes its order within its column — **and, from 2026-09-21, its vertical position too, which is the half of the pin that keeps the reader's place while a map is being written** | `test_within_layer_order_is_stable` in `layout.test.ts`; growth › `test_a_tile_keeps_its_row_while_the_map_grows` |
