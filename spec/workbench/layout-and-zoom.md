...
### B3 — A later tile does not move an earlier one's row
...
**Which is what the chapter always said, and not what the code did** *(amended 2026-09-21, decision record 0024)*. `readPositions` in `frontend/src/graph/elkGraph.ts` held **x as well as y**, in its own words *"a tile that had a position keeps that position, to the pixel"*, and the pins were dropped only when a box changed height — measured at **zero pin drops in 25, 14 and 12 layout passes** over three real generated maps (analyst UB, 2026-09-21). Holding x is how a claim ends up drawn in a column it no longer belongs to, and an arrow into it drawn pointing backwards: on the Verify door the destination claim arrives with no causes, is pinned in column 0 beside the hypothesis, and gains four arrows into it, three of them from claims placed to its right. **So the pin holds the row and the pin does not hold the column.** A tile's column is re-read from the arrows on every pass; when a later arrow proves a claim belongs further right, the claim goes there and keeps its row. And a map that was written while a reader watched is laid out once, whole, at the moment the run stops — the same moment the canvas already re-frames at — and never again. `streaming-growth.md` INV-workbench.64 owns that rule and its measurements; this chapter owns only the fact that a pin is about a row.
...
| **INV-workbench.22** | For every map and every claim added strictly downstream of the tiles already placed, every already-placed tile's coordinates are identical before and after. **Unchanged 2026-09-21**: a claim added strictly downstream cannot pull an earlier claim into a different column, so this invariant covers exactly the case where nothing moves. What *can* move a column is an arrows-only proposal, or a claim that arrived with nothing pointing at it — `streaming-growth.md` INV-workbench.64 | `test_pinned_tiles_keep_their_positions` in `layout.test.ts` |
| **INV-workbench.23** | For every map and every re-layout of it, no already-placed tile changes its order within its column. **And, from 2026-09-21, for every re-layout while a map is still growing, no already-placed tile changes its vertical position either** — that is the half of the pin that keeps the reader's place while a map is being written. The scope is deliberate: the two moments a pin is dropped altogether are a branch opening (B7, a different map laid out whole) and the settle when a run stops (`streaming-growth.md` INV-workbench.64), and at both of those a row may move | `test_within_layer_order_is_stable` in `layout.test.ts`; growth › `test_a_tile_keeps_its_row_while_the_map_grows` |
