...
13. **VR13** — The map grew rather than appeared: a reserved rectangle stood where the next claim would go, every proposal the rules refused is on screen in the validator's own words, and the receipt says what the run cost. **No tile that was already placed changed row, and no arrow points backwards** — a tile may change column when a later arrow proves it belongs further right, and the whole map takes its final places once at the moment the run stops (decision record 0024, 2026-09-21; this line read *"no tile that was already placed moved when a later one arrived"*). **And the foot of the map says, in words, what the run is doing** — with a count of seconds since the last event on a live run, and none on a replay (decision record 0023). Checked on a picture of a live **Verify** run, not only of a replay.
