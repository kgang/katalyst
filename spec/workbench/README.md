...
2. **VR2** — There is no spinner, no pop-up, and no dialog that has to be dismissed. **Checked on a picture of a run that is waiting, not only of one that has finished** (decision record 0023, 2026-09-21): the wait is where a spinner would be, so a picture taken mid-wait is the only one that can find one. A count of seconds beside a sentence is not a spinner; anything that turns, pulses or sweeps is.
...
13. **VR13** — The map grew rather than appeared: a reserved rectangle stood where the next claim would go, every proposal the rules refused is on screen in the validator's own words, and the receipt says what the run cost. **While the map was still growing, no tile that was already placed changed row, and at no moment does an arrow point backwards** — a tile may change column when a later arrow proves it belongs further right, and the whole map takes its final places once at the moment the run stops, where rows move too (decision record 0024, 2026-09-21; this line read *"no tile that was already placed moved when a later one arrived"*). A picture taken after the settle is checked for the arrows, not for the rows. **And the foot of the map says, in words, what the run is doing** — with a count of seconds since the last event whenever the screen is waiting on the server, and none on a replay (decision record 0023). Checked on a picture of a live **Verify** run, not only of a replay.
