...
**Belief.** `{p, lo, hi, owner}` with `owner ∈ {model, user, market}` and `0 ≤ lo ≤ p ≤ hi ≤ 1` (INV-7). The three owners are stored and rendered separately and never averaged (INV-11). Rendered at **two significant figures and nothing else**. **`lo` and `hi` are not a range any more** *(2026-09-22; decision record 0028, one likelihood per claim, computed once)*. They used to be the 10th and 90th percentiles of the likelihood itself — how sure we are of the number, not how much the world can move — and every one of them now equals `p`. The browser drops them at the wire, nothing prints them, and one later pull request takes all three fields down to one. **Never put a number in them again.**
...
**World.** A base graph with a branch applied and beliefs propagated. Replayable from `(base_id, branch, seed)` (INV-5, NFR-2). The base world is the empty branch. **Built 2026-09-22** (decision record 0016): a claim's number is the chance it comes out true by its own deadline, and its series is how that chance stands on each day — rising for an event, able to fall for a state. A world carries **one reading per claim**, computed once (decision record 0028); the `worlds` and `versions` counts it still reports read `0` and `1` and are on their way off the wire.

**Diff.** Between two worlds: per proposition, `unchanged` · `shifted` (with before → after) · `added` · `killed`; plus a ranked list of terminal deltas and a one-line natural-language summary. **`killed` means forced false, and nothing else** (decided 2026-09-17) — never "its number got small", and never "no path reaches it from the hypothesis any more", which is a fact about the *path* and is reported by the Inspector's path bar in those words. The ranking is the size of the move times the weakest provenance weight on the **best-backed route** from a differing edit's subject to that ending: over every such route, the one whose weakest arrow is strongest. Two factors, and neither was ever a version, so both are untouched by the record below. *(Amended 2026-09-22; decision record 0028. Two columns stood beside the ranking and are **retired**: the width of the new number's range, and* agreement *— the share of versions of the map that moved the same way. There are no versions and no range, so there is nothing for either to be a share or a width of. **`shifted` now has one condition**: the claim moved by `.005` or more.)*

**Version of the map.** **Retired 2026-09-22** (decision record 0028, Kent's decisions-note row R48). A version was one coherent set of numbers this model would have stood behind — every claim's likelihood drawn from its own stated range at once — and the engine ran two thousand of them, with eight sampled worlds under each, so that the range on a computed number was the spread across versions. **All of it is gone**: one likelihood per claim, computed once, and no range anywhere. The word is kept here so a reader who knew it is told it is not a word this product uses; the `versions` count the engine still reports reads `1` and `worlds` reads `0`, and both leave the wire with the range fields. **Nothing may draw a version again.**

**Agreement.** **Retired 2026-09-22** (decision record 0028). It was the share of versions of the map in which a change moved the same way — computed, never self-reported — shown as the change list's own column headed *same direction*, and the second half of the test that decided whether a claim had moved. It was a share **of versions**, and there are none, so it went with them: the column is off the screen, the `agreement` field is always absent, and **nothing may put a number in it again**. The width column beside it, headed *how firm* and carried in the field `range_width`, went the same way and for the same reason — there is no range to be the width of.

**The word itself is still free**, and that is now its whole entry. It is reserved for a **run-to-run** number — how far several independent generation runs agreed on a claim's likelihood — if one is ever earned. Nothing computes one: decision record 0015 ships the range the model stated and says *not yet* to an ensemble, and decision record 0021 closed the one condition that would have reopened it. If such a number ever exists: computed, never self-reported, and wherever both could be meant, say which.
...
**Ceiling.** A quartered Kelly — a quarter of the bet size that would make capital grow fastest given a stated edge — worked out at the unfavourable end of the model's stated range, shown greyed and labelled *never size to this*. It is never a recommended size; the reader's risk budget is the only number that sets one. *(Amended 2026-09-22; decision record 0028: there is no range, so the unfavourable end of it is the likelihood itself. The rule is unchanged and reads one number instead of two.)*
...
**Tile.** A proposition's on-canvas card. **Port.** A typed input/output on a tile. **Wire.** A link's on-canvas rendering. **Inspector.** The persistent side panel; never a modal. **Delta rail.** The ranked terminal-delta list beside a diff. *(Amended 2026-09-22; decision record 0028: it had **two side columns**, headed* how firm *— the width of the new number's range — and* same direction *— the share of versions of the map that moved the same way, the field called `agreement`. Both are **retired**, because there is no range and there are no versions. The rail is the ranked list and its moves, and nothing beside them.)* **Launchpad.** The empty state with the four seeded examples.
...
**Retracted.** **Gone 2026-09-22** (decision record 0017). This was the one **derived** badge — no button produced it. **Retracted · date · by "…"** appeared on a claim that had been supposed true and was then pushed back down by a later edit, naming the edit responsible and the day it landed. Automatic retraction left the engine with the flip: nothing on this product undermines a claim by itself, the list of retractions a world reports is always empty, and **no surface may draw this badge again**. What takes its place is the **state** — a claim that holds over a stretch of time and can stop — whose tile says the day it stopped holding and names the claim that ended it (UX-14). A claim the user supposed true simply stays supposed until another edit changes it. **Every badge this product draws is now produced by a button somebody pressed.**
...
| No number of your own yet | **—** | "You have not given a number for this claim." *(The words **add yours** stood beside the dash and shared a line with the range; both went on 2026-09-22, R48, and the tile no longer draws an empty column at all.)* |
...
| A venue quotes this claim but publishes no interval | the number | "The venue publishes one price. What the two sides of its book differ by is what dealing costs, and it is shown there." *(Amended 2026-09-22; Kent's R48 and decision record 0028. This row said the card printed the number **with no range**; no number anywhere carries one now, so this is no longer a case that differs from any other. The row is kept because the sentence is still the true answer to “why is there only one number here?” from a reader looking at a price.)* |
...
*Prediction* (we model arguments, not oracles) · *scenario* (ambiguous between branch and world) · *edge* when we mean a link (reserve *edge* for the model-against-venue difference defined under *The trade* above) · *invalidation*, retired 2026-09-21 (decision record 0019) — say *what to watch* for the computed list and *your exit* for the price the reader types, and never call either a stop-loss · *node* in UI copy (say tile or proposition) · *confidence*, at all — there is no confidence field on anything. A link's standing is its `provenance` (a receipt we write) and its rationale *(amended 2026-09-22; decision record 0028: the range on the belief stood third in that list, and there is no range)*. **And, on screen, six more words are out from 2026-09-22 (Kent's R48):** *range*, *band*, *interval*, *uncalibrated*, *version* or *versions* of the map, and *worlds* — except in the address `/api/worlds`, which is a route rather than a word about the multiverse. A number for how much things computed independently agreed is called *agreement* — defined under *The multiverse* above — and is computed, never self-reported — and nothing computes one.
