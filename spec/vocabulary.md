...
**Belief.** `{p, lo, hi, owner}` with `owner ∈ {model, user, market}` and `0 ≤ lo ≤ p ≤ hi ≤ 1` (INV-7). The three owners are stored and rendered separately and never averaged (INV-11). **On screen a belief is one likelihood at two significant figures, and `lo` and `hi` are not drawn anywhere** *(Kent, 2026-09-22, R48: one likelihood per claim, no range anywhere, ever)*. The browser drops both at the wire; the shape above is still what the server sends, and **the engine half of the cut — this entry's own arithmetic, and records 0005 and 0014 — is the stack 05 session's**, so this line is true of the wire and false of the screen until it lands.
...
**Tile.** A proposition's on-canvas card. **Port.** A typed input/output on a tile. **Wire.** A link's on-canvas rendering. **Inspector.** The persistent side panel; never a modal. **Delta rail.** The ranked terminal-delta list beside a diff. *(It had two side columns — **how firm**, the width of the new number's range, and **same direction**, the share of versions of the map that moved the same way. Both were cut on 2026-09-22, R48; a row is the ending, its number before and after, and the direction. The word *agreement* stays free for a run-to-run number, if one is ever earned.)* **Launchpad.** The empty state with the four seeded examples.
...
| A venue quotes this claim but publishes no interval | the number | "The venue publishes one price. What the two sides of its book differ by is what dealing costs, and it is shown there." *(This row said the card printed the number **with no range**; after 2026-09-22 (R48) no number anywhere carries one, so this is no longer a case that differs from any other.)* |
